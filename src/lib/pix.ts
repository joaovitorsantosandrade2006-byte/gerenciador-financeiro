...
function crc16(str: string): string {
  ...
  for (let i = 0; i < str.length; i++) {
    crc = ((crc << 8) ^ CRC16_XMODEM_TABLE[((crc >> 8) ^ str.charCodeAt(i)) & 0xFF]) & 0xFFFF;
  }
...
export function generatePixPayload(data: PixData): string {
  ...
  merchantAccount += generateEMVField('01', data.pixKey);
  ...
  // Merchant Name
  payload += generateEMVField('59', data.merchantName);
  
  // Merchant City
  payload += generateEMVField('60', data.merchantCity);
  
  // Additional Data Field
  if (data.description) {
    const additionalData = generateEMVField('05', data.description);
...
export function generatePixQRCode(payload: string): string {
  // Retorna a URL do QR Code usando API pública
